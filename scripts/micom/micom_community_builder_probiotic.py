...
makedirs("probiotic_models", exist_ok = True)
...
taxonomy["file"] = taxonomy.file.apply(
    lambda ids: ["../../agora_genus+species/" + i for i in ids.split("|")]
)
taxonomy["name"] = [taxonomy.loc[i,'genus'] if pd.isna(taxonomy.loc[i,'species']) else taxonomy.loc[i,'species'] for i in taxonomy.index]
...
def build_and_save(args):
	try:
		s, tax,prefix = args
		filename = "probiotic_models/" + prefix+ "_" + s + ".pickle"
		if isfile(filename):
			 return

		com = Community(tax, id=s, progress=False)
		ex_ids = [r.id for r in com.exchanges]
		logger.info("%d/%d import reactions found in model.",diet.index.isin(ex_ids).sum(),len(diet),)
